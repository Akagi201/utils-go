...
func TestBinaryEncodeComplex(t *testing.T) {
	b, err := binary.Marshal(s1v)
	assert.NoError(t, err)
	assert.Equal(t, svb, b)
	s := &s1{}
	err = binary.Unmarshal(svb, s)
	assert.NoError(t, err)
...
func TestMarshalNonPointer(t *testing.T) {
	...
	s := S{A: 1}
	data, err := binary.Marshal(s)
	if err != nil {
		t.Fatal(err)
	}
	var res S
	if err := binary.Unmarshal(data, &res); err != nil {
		t.Fatal(err)
	}
