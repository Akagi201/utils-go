...
func TestToArgs_Bool(t *testing.T) {
	asserts := []assert{
		assert{config{"flag": false}, nil, []string{"-flag=false"}},
		assert{config{"flag": true}, nil, []string{"-flag=true"}},
	}
...
func TestToArgs_List(t *testing.T) {
	c := config{"flag": []interface{}{"value1", "value2"}}
...
func TestToArgs_Positions(t *testing.T) {
	asserts := []assert{
		...
		assert{
			config{"options": map[string]interface{}{"flag": "value"}},
			[]string{"options"},
			[]string{"-flag", "value"},
		},
		// options in general/options section.
		assert{
			config{"general": map[string]interface{}{"options": map[string]interface{}{"flag": "value"}}},
...
func TestToArgs_LongHyphenOption(t *testing.T) {
	asserts := []assert{
		assert{config{"f": "1"}, nil, []string{"-f", "1"}},
		assert{config{"flag": "1"}, nil, []string{"--flag", "1"}},
	}
...
func TestToArgs_BoolValueOption(t *testing.T) {
	asserts := []assert{
		assert{config{"flag": false}, nil, nil},
		assert{config{"flag": true}, nil, []string{"-flag"}},
	}
